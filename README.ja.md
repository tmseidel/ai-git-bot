...
🌐 言語選択：[English](README.md) · [中文](README.zh.md) · [한국어](README.ko.md) · **日本語**
...
| **[イシュー → コード](doc/CODING_AGENT.md)** | コーディングエージェントのイシューワークフローを持つボットにイシューを割り当て | プルリクエスト |
| **[イシュー → 改善](doc/WRITER_AGENT.md)** | ライターエージェントのイシューワークフローを持つボットにイシューを割り当て | 受け入れ基準付き構造化イシュー |
| **[イシュー → 適切な担当者](doc/ISSUE_TRIAGE.md)** | イシューのオープン、またはイシュートリアージのイシューワークフローを持つボットへの割り当て | 理由コメントとイシュー割り当て |
| **[イシューワークフロー](doc/ISSUE_WORKFLOWS.md)** | ボットごとに設定可能 | PR ワークフローから独立したプラガブルなイシュー割り当て動作 |
...
イシューをオープンする（またはイシュートリアージのイシューワークフローを持つボットに割り当てる）と、ボットが担当者を決定します。

編集可能なルーティングプロンプトを使って、ボットは：

1. イシューを読む
2. 設定されたアカウントから担当者を1人だけ選択 — 人、ボット、または `none`
3. 理由をコメントとして投稿
4. イシューを割り当て（`none` の場合は未割り当てのまま）
...
- 🔒 `APP_ENCRYPTION_KEY` 設定時の AES-256-GCM シークレット暗号化
- 🤖 マルチプロバイダ AI サポート
...
- 🐳 Docker ファーストデプロイメント
- 🌍 エンドツーエンド セルフホスト可能
- ♻️ AI プロバイダーが過負荷（HTTP 503）を報告した際のバックオフ付き自動リトライ — 影響を受ける PR や Issue には次回試行時刻が通知されます
