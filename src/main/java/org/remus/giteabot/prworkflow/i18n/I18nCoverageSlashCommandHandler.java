...
/**
 * Detects and dispatches the {@code @bot regenerate-i18n [instruction]} slash
 * command for the {@link I18nCoverageWorkflow}.
 *
 * <p>Mirrors {@code ReadmeSyncSlashCommandHandler} / {@code E2eTestSlashCommandHandler}:
 ...
 * {@code i18n-coverage} workflow via the orchestrator, threading any trailing
 * free-text as the {@link PrWorkflowContext#HINT_I18N_COVERAGE_GUIDANCE} hint.
 * The detection logic is unchanged; the trailing text is only used as extra
 * guidance for the LLM-generated translations. The command is ignored (returns
 * {@code false}) when the bot does not have the workflow enabled, so the regular
 * comment handling still runs.</p>
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class I18nCoverageSlashCommandHandler {

    private static final Pattern COMMAND_PATTERN = Pattern.compile(
            "(?im)(?:^|\\s)@\\S+\\s+regenerate-i18n\\b\\s*(.*)$");
     ...
     *         {@code regenerate-i18n} slash command and dispatched;
    ...
    public boolean tryHandle(Bot bot, WebhookPayload payload) {
        ...
        if (!isEnabledOnBot(bot)) {
            log.info("[Bot '{}'] i18n-coverage slash command ignored — workflow not enabled", bot.getName());
            return false;
        }

        addEyesReaction(bot, payload);
        log.info("[Bot '{}'] i18n-coverage slash command detected (guidance='{}'), dispatching {} workflow",
                bot.getName(), abbreviate(guidance, 80), I18nCoverageWorkflow.KEY);
        try {
            hydratePullRequest(bot, payload);
        } catch (RuntimeException e) {
            log.warn("[Bot '{}'] Could not hydrate PR for regenerate-i18n: {}", bot.getName(), e.getMessage());
        }
        try {
            Map<String, String> hints = guidance.isBlank()
                    ? Map.of()
                    : Map.of(PrWorkflowContext.HINT_I18N_COVERAGE_GUIDANCE, guidance);
            orchestrator.run(bot, payload, I18nCoverageWorkflow.KEY, hints);
        } catch (RuntimeException e) {
            log.warn("[Bot '{}'] i18n-coverage slash command dispatch failed: {}",
    ...
    private boolean isEnabledOnBot(Bot bot) {
        ...
        try {
            return selectionService
                    ...
                    .contains(I18nCoverageWorkflow.KEY);
        } catch (RuntimeException e) {
            log.debug("[Bot '{}'] i18n-coverage enabled-check failed: {}", bot.getName(), e.getMessage());
            return false;
        }
    }
