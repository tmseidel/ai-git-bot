...
/**
 * Lightweight chat-and-dispatch loop for the {@link UnitTestAuthorAgent}.
 *
 * <p>Mirrors {@code E2eAgentRunner} one-to-one — native function calling when
 * the provider supports it, transparent legacy-envelope fallback otherwise —
 * but dispatches {@code unit-test-write} calls to the
 * {@link UnitTestToolExecutor} against the real repository checkout instead of
 * a sandboxed E2E workspace. It is intentionally separate from the
 * {@code AgentLoop} (which assumes an {@code AgentSession}, branch switching
 * and the full coding tool router) because the author only needs chat turns
 * plus a single write tool.</p>
 */
@Slf4j
public final class UnitTestAgentRunner {
    ...
    public UnitTestAgentRunner(AiClient aiClient,
        ...
        this.agentLabel = agentLabel == null ? "unit-test-author" : agentLabel;
    ...
    public record Result(String lastAssistantText,
        ...
        public long writeCount() {
            return toolInvocations.stream()
                    .filter(i -> "unit-test-write".equalsIgnoreCase(i.toolName()))
