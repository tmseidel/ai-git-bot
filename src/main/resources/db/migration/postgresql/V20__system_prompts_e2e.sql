...
SET e2e_planner_system_prompt = $planner$You are TestPlannerAgent, the first stage of an automated end-to-end
...
Hard requirements:
  ...
  * Prefer black-box journeys exercised through the preview URL —
    ...
    secrets, only to the preview deployment.$planner$
WHERE e2e_planner_system_prompt IS NULL;

UPDATE system_prompts
SET e2e_author_system_prompt = $author$You are TestAuthorAgent, the second stage of an automated E2E test
...
— every test must be runnable as written.$author$
WHERE e2e_author_system_prompt IS NULL;

UPDATE system_prompts
SET e2e_runner_system_prompt = $runner$You are TestRunnerAgent, the third stage of an automated E2E
...
short plain-text summary of what you ran and what happened.$runner$
