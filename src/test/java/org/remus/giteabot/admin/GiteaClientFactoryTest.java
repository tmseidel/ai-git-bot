...
class GiteaClientFactoryTest {
    ...
    @Test
    void replacementTokenClient_doesNotDecryptOrCacheSubmittedToken() {
        GitIntegration integration = integration(1L, "HTTP", GitTransport.HTTP);

        var credentials = factory.createApiClient(integration, "replacement-token").getCredentials();

        assertThat(credentials.token()).isEqualTo("replacement-token");
