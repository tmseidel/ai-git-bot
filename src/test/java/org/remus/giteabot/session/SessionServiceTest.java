...
class SessionServiceTest {
    ...
    @Test
    void updatePrContext_updatesChangedFields() {
        ReviewSession session = new ReviewSession("owner", "repo", 1L, null);
        when(repository.save(any(ReviewSession.class))).thenAnswer(invocation -> invocation.getArgument(0));

        ReviewSession updated = sessionService.updatePrContext(session, "New title", "New body");

        assertEquals("New title", updated.getPrTitle());
        assertEquals("New body", updated.getPrBody());
        verify(repository).save(session);
    }

    @Test
    void updatePrContext_withoutChanges_skipsSave() {
        ReviewSession session = new ReviewSession("owner", "repo", 1L, null);
        session.setPrTitle("Title");
        session.setPrBody("Body");

        ReviewSession updated = sessionService.updatePrContext(session, "Title", "Body");
