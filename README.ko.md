...
누구도 이런 아이디어에 동의하지 않습니다.

* 문제는 불편하다는 점입니다.
* 반복적이라는 점입니다.
* 로드맵에 자주 올라가지 않는다는 점입니다.
* 그리고 마감이 임박하면 가장 먼저 사라진다는 점입니다.
...
PR 을 봇에 할당하면 변경 사항에 대한 Playwright 테스트 스위트(세트)를 생성하고 프리뷰 환경을 배포하며 테스트를 실행한 후 결과를 PR 에 게시할 수 있습니다.
...
| 워크플로우 | 트리거 | 결과 |
|-----------|----------|------|
| **PR 리뷰** | PR 열림 또는 리뷰 재요청 | 리뷰 코멘트 및 결과 |
| **대화형 Q&A** | PR 댓글에서 `@bot` 멘션 | 컨텍스트 인식 대화 |
| **이슈 → 코드** | 이슈를 코드 봇에 할당 | 풀 리퀘스트 |
| **이슈 → 개선** | 이슈를 작성 봇에 할당 | 수용 기준이 있는 구조화된 이슈 |
| **유닛 테스트 생성** | PR 열림 또는 명령 트리거 | 브랜치에 커밋된 생성 테스트 |
| **풀스택 QA** | PR 열림 | 프리뷰 환경에서 실행된 Playwright 스위트 |
| **PR 재리뷰** | 강제 푸시 또는 리뷰 요청 | 업데이트된 분석 |
| **워크플로우 자동화** | Git 이벤트 | 엔지니어링 잡무 자동화 |
...
AI-Git-Bot 은 코드 주변 작업을 자동화하여 팀에 제공합니다.
...
AI-Git-Bot 은 Git 플랫폼과 AI 제공자 사이의 게이트웨이 역할을 합니다.
